...
The result is a data.table with `r dim(AllDT)[2]` columns and `r dim(AllDT)[1]` rows. The last step is to add variable names. I import the names for the feature variables from features.txt and manually add the lables "subject" and "activity" for the first two columns. However, the names in features.txt contain strings that cause problems in R like "(", ")", and "-". So I delete or replace them.
